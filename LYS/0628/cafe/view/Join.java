		...
		public void actionPerformed(ActionEvent e) {
			...
			if (isAvailableID && isAvailablePWD && isAvailableName) {
				if (mc.isDuplicatedMember(id)) {
					// 중복된 아이디 에러메시지를 보낸다.
					JOptionPane.showMessageDialog(null, "중복된 아이디 입니다! 다시 입력해주세요!", "회원가입 실패",
							JOptionPane.ERROR_MESSAGE);
				} else {
					// 성공 Dialog
					...
					String filePath = "C:\\Users\\cndjr\\Documents\\Member.txt";
										//경로를 바꾸어야할거같아요
				try {
					BufferedWriter writer = new BufferedWriter(new FileWriter(filePath, true)); 

					//사용자가 join에 입력한 값을 저장하기
			        writer.write(String.format("%s,%s,%s,%s,%s\n", id, pwd, name, address, birthday));
			        writer.close();
					...
					changePanel(new LoginAfter(mf, newMember));
				}
			}
		}
	}
